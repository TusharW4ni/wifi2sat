...
ELEV_MASK = 20.0           # deg; below this, tropo/iono bending makes the apparent
                           # LOS drift nonlinearly vs the geometric orbital v_perp,
                           # and multipath rises. Exclude such sats as signal sources.
TARGET_EPOCHS = 100
FS = 10
...
def extract_signal(phases, ref_key=None, elevations=None, los=None,
                   fs=FS, n_epochs=TARGET_EPOCHS, trend_deg=2, elev_mask=ELEV_MASK):
    ...
    elev_mask:  drop signal sats below this elevation (deg); needs `elevations`.
                Set None to disable.
    returns dict:
    ...
    if elevations is not None and elev_mask is not None:
        clean = [k for k in clean if elevations.get(k, -91) >= elev_mask]
    if len(clean) < 2:
        raise ValueError(f"need >=2 clean sats above {elev_mask} deg, got {len(clean)}")
