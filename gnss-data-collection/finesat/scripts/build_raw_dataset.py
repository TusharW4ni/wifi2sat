...
def process_file_raw(filepath):
    """Processes a file to get the Raw Baseline features (N=6 format)."""
    ...
    for i in range(MAX_TARGET_SATS):
        if i < len(target_keys):
            ...
            # RAW BASELINE: Just difference, NO polynomial detrending
            raw_diff = tgt_signal - ref_signal
            features = extract_13_features(raw_diff)
        else:
...
def main():
    ...
    print(f"Processing {len(files)} files for RAW Baseline...")
    
    for f in files:
        # Extract label from filename (e.g., "push" from "push-260430-181437.rtcm")
